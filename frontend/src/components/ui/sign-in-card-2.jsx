...
export default function SignInCard2({ mode = 'login' }) {
  ...
  const validate = () => {
    ...
    if (!isLogin) {
      ...
      else if (fullName.length < 3) newErrors.fullName = 'Must be at least 3 characters.';
    }
  ...
  const getInputClass = (fieldName) => `
    w-full bg-white/50 border text-slate-900 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 transition-all placeholder:text-slate-400 shadow-sm disabled:opacity-50
    ${errors[fieldName] 
      ? 'border-red-400 focus:ring-red-400/20 focus:border-red-500 bg-red-50/10' 
  ...
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="visible"
      className="w-full max-w-[420px] mx-auto z-10 p-8 sm:p-10 rounded-[2rem] bg-white/70 backdrop-blur-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-y-auto max-h-[90vh] custom-scrollbar"
    >
      <div className="text-center mb-8">
        <motion.div variants={itemVariants} className="inline-flex items-center justify-center gap-2 mb-6">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-900 to-slate-700 flex items-center justify-center text-white shadow-lg">
        ...
        <motion.p variants={itemVariants} className="text-sm font-medium text-slate-500">
          {isLogin ? "Sign in to your JusticeAI account" : "Join JusticeAI to analyze documents instantly"}
        </motion.p>
      </div>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="pt-3">
          <motion.button
            ...
            type="submit"
            disabled={isLoading}
            className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3.5 rounded-xl transition-all shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 disabled:opacity-70 flex items-center justify-center gap-2 group relative overflow-hidden"
          >
            {isLoading ? (
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-4 mt-6">
          ...
          <motion.button
            ...
            type="button"
            disabled={isLoading}
            className="w-full bg-white/60 border border-slate-200 text-slate-700 hover:bg-white font-semibold py-3.5 rounded-xl transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10 flex items-center justify-center gap-3 disabled:opacity-50 backdrop-blur-sm"
          >
      ...
      <motion.p variants={itemVariants} className="mt-8 text-center text-sm text-slate-500 font-medium">
        ...
        <Link 
          ...
          className="text-slate-900 font-bold hover:underline underline-offset-4 decoration-2 decoration-slate-300 transition-all"
        >
